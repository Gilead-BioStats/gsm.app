...
# Each analysis contains the five-step data pipeline
lAnalyzed$Analysis_kri0001# Example: AE KRI
```
...
# Creating the fnFetchData Function and App Deployment
...
The `fnFetchData` function should accept parameters that allow filtering by group, metric, and other relevant criteria:

```{r create-fnfetchdata}
# Create the data fetch function
fnFetchData <- function(strDomainID = 
                        c("AE", "ENROLL", "LB", "PD", "SDRGCOMP", 
                          "STUDCOMP", "SUBJ", "DATACHG", "DATAENT", "QUERY"),
                      strGroupID = NULL,
                      strGroupLevel = "Site",
                      strSubjectID = NULL,
                      dSnapshotDate = NULL) {
...
For development and testing, run the app locally:
run_gsm_app() creates a Shiny app to explore a set of clinical trial data. The app facilitates exploration of the data by allowing the user to click to diver deeper into aspects of the data.
