test_that("mod_TimeSeries_UI creates the expected UI", {
  test_result <- mod_TimeSeries_UI("myID")
...
test_that("mod_TimeSeries_UI uses title when it's supplied", {
  test_result <- mod_TimeSeries_UI("myID", strPlotTitle = "myTitle")
  expect_identical(
...
test_that("mod_TimeSeries_Server sets selected site", {
  dfResults <- FilterbyMetricID(
    sample_dfResults[sample_dfResults$GroupLevel == "Site", ],
    "Analysis_kri0001"
  )
